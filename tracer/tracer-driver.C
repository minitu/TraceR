...
char tracer_config[256]; // Tracer input filename
char trace_dir[256]; // Folder where traces are located

CoreInf *global_rank; // Mapping from core to job ID and process ID
JobInf *jobs;
int default_mapping;
int total_ranks;
tw_stime *jobTimes;
...
{
  g_tw_ts_end = s_to_ns(60*60); // Upper bound of ROSS virtual time
  tw_opt_add(app_opt); // Add TraceR options to ROSS
  ...
  if (argc < 2 && rank == 0) {
    printf("\nUSAGE: \n");
    printf("\tmpirun -n <arg> ./TraceR --sync=3 --nkp=16 --extramem=100000 "
        "--max-opt-lookahead=1000000 --timer-frequency=1000 "
        "-- <net.conf> <tracer.conf> <trace dir>\n\n");
    MPI_Abort(MPI_COMM_WORLD, 1);
  }

  // Load network configuration to CODES
  configuration_load(argv[1], MPI_COMM_WORLD, &config);

  strncpy(tracer_config, argv[2], strlen(argv[2]) + 1);
  strncpy(trace_dir, argv[3], strlen(argv[3]) + 1);

  if (rank == 0) {
    printf("Network config file: %s\n", argv[1]);
    printf("TraceR config file: %s\n", tracer_config);
    printf("Trace directory: %s\n", trace_dir);
  }

  // Register LPs to CODES
  model_net_register();

  int num_nets;
  int *net_ids;

  // Configure CODES model-net
  ...
  if (dump_topo_only || strcmp("NA", globalIn) == 0) {
    if (rank == 0) printf("Using default linear mapping of jobs\n");
    default_mapping = 1;
  }
  else {
    if (rank == 0) printf("Reading global mapping file: %s\n", globalIn);
    default_mapping = 0;

    if (rank == 0) {
      FILE *gfile = fopen(globalIn, "rb");
      if (gfile == NULL) {
        printf("Unable to open global rank file %s\n", globalIn);
        MPI_Abort(MPI_COMM_WORLD, 1);
      }
  ...
  if (total_ranks > num_servers) {
    if (rank == 0)
      printf("Job requires %d servers, but the topology only contains %d\n", total_ranks, num_servers);
    MPI_Abort(MPI_COMM_WORLD, 1);
  }
    ...
    if (next == 'E' || next == 'e') {
      ...
      if (rank == 0)
        printf("Will make all events with name %s run for %lf s for job %d;"
            "if scale_all, events will be scaled down\n", eName, etime, jobid);
  ...
  for (int i = 0; i < num_jobs && !dump_topo_only; i++) {
    ...
    if (default_mapping) {
      for (int local_rank = 0; local_rank < num_workers; local_rank++, ranks_till_now++) {
      ...
      if (rank == 0) {
        printf("Loading map file for job %d from %s\n", i, jobs[i].map_file);
        FILE *rfile = fopen(jobs[i].map_file, "rb");
        if (rfile == NULL) {
          printf("Unable to open local rank file %s\n", jobs[i].map_file);
          MPI_Abort(MPI_COMM_WORLD, 1);
        }
  ...
  for (int i = 0; i < num_jobs && !dump_topo_only; i++) {
    if (rank == 0) printf("Loading trace summary for job %d from %s\n", i, jobs[i].traceDir);
  ...
  for (int i = 0; i < num_jobs && !dump_topo_only; i++) {
    if (rank == 0) printf("Read global definition for job %d from %s\n", i, jobs[i].traceDir);
    jobs[i].allData = new AllData;
    jobs[i].reader = readGlobalDefinitions(i, jobs[i].traceDir, jobs[i].allData);
  }
#endif

  // Pass control to ROSS, which eventually calls proc_init()
  tw_run();

  if (lp_io_flush(handle, MPI_COMM_WORLD) < 0) return -1;

  tw_stime* jobTimesMax = (tw_stime*) malloc(num_jobs * sizeof(tw_stime));
  MPI_Reduce(jobTimes, jobTimesMax, num_jobs, MPI_DOUBLE, MPI_MAX, 0, MPI_COMM_WORLD);
  ...
  // Report end-of-simulation model-net statistics
